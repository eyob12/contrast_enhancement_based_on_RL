...
if __name__ == '__main__':
	...
	def save_images(deteriorated, enhanced, idx):
		im1 = Image.fromarray(deteriorated)
		im2 = Image.fromarray(enhanced)
		im1.save('training_result/deteriorated_'+ str(idx).zfill(2)+'.png')
		im2.save('training_result/enhanced_'+ str(idx).zfill(2)+'.png')
	...
	for itr in range(EPISODES):
		...
		while not gui.skip:
			...
			gui.flag, gui.log = False, False #button waiting
			show_results()
			...
			gui.total_ave.append((np.sum(gui.reward) +
				np.sum(gui.punishment)) / (gui.itr + 1))
			#6. update Q-table
			ql.learn(s,a,r,s_n)
			gui.itr += 1
			s = s_n
			...
			if gui.log:
				save_images(tmp, rca.imt, int(itr))
			rca.imd = rca.imt
			# gui.satisfy += 1
		#............go to test
		if not gui.mode:
	...
	for idx in test_index:
		...
		show_results()
		tstsv = Image.fromarray(rca.imt)
		tstsv.save('test_result/result_'+str(idx).zfill(2)+'.png')
		if gui.mode:
			...
